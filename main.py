...
def make_doc_info() -> bytes:
    ...
    cs += struct.pack("<I", 0)
    cs += struct.pack("<bb", 0, 0)
    cs += struct.pack("<IIII", 0, 0, 0x00808080, 0)
    cs += struct.pack("<H", 0)
    cs += struct.pack("<I", 0)
    assert len(cs) == 72
    ...
    ps = struct.pack("<I", 0)
    ps += struct.pack("<iiii", 0, 0, 0, 0)
    ps += struct.pack("<ii", 0, 0)
    ps += struct.pack("<i", 200)
    ps += struct.pack("<HHH", 0, 0, 0)
    ps += struct.pack("<hhhh", 0, 0, 0, 0)
    ps += struct.pack("<III", 0, 0, 200)
    assert len(ps) == 54
